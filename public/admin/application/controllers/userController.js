adminApp.controller('userController',function($scope,$compile,dataService){
	...
	$scope.search={};
	$scope.limit=2;
	$scope.page=1;// variable to hold page no for pagination
	...
	$scope.datalist.searchData=function(){
		...
		if($scope.search.searchby!=null && $scope.search.searchkey!=null){
			...
			$scope.search={};
			dataService.callAjax({searchParam:searchObj,limitParam:{}},$scope.title+'/getData',function(data){
				if(data){
					$scope.tableData=data;
